    ...
    HTMLElement.prototype.Sparnatural = function( options ) {
		...
		this.getMaxVarIndex = function() {
			...
			for (var i = 0; i < this.components.length; i++) {
				...
				if(endVarName && endVarName.split("_").length > 1) {
					index = parseInt(endVarName.split("_")[1]);
					if(index > max) {
						max = index;
					}	
				}
		...
		function initForm(form) {	
			...
			if (settings.onSubmit instanceof Function) {
				SubmitSection = '<div class="submitSectionWrapper" style="background: rgba('+settings.backgroundBaseColor+');"><div class="submitSection"><a class="submitDisable">'+UiuxConfig.ICON_PLAY+'</a></div></div>' ; 
			}
		...
		this.init = function init(reload = false) {
			...
			if (
				...
				if (
					...
					endLabel = langSearch.Select+" :" ;
				} else if(this.widgetType == Config.BOOLEAN_PROPERTY) {
					endLabel = "" ;
				} else {
					endLabel = langSearch.Find+" :" ;
				}
			}
		...
		this.createWidgetComponent = function createWidgetComponent(widgetType, objectPropertyId, rangeClassId) {
			switch (widgetType) {
			  ...
			  case Config.LIST_PROPERTY:
			    // defaut handler to be used
			    handler = this.settings.list;
			    
			    // to be passed in anonymous functions
			    theSpecProvider = this.specProvider;

			    // determine custom datasource
			    datasource = this.specProvider.getDatasource(objectPropertyId);
			  ...
			  case Config.AUTOCOMPLETE_PROPERTY:
			    handler = this.settings.autocomplete;
			    // to be passed in anonymous functions
			    theSpecProvider = this.specProvider;
