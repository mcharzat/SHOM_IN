 ...
 export class StartClassGroup extends GroupContenaire { 
	...
	onCreated() {
		...
		$(this.html).find('select.input-val').on(
			...
			this.SparnaturalComponents.eventProxiCriteria
		);
	...
	onChange() {
		...
		var parentOrSibling = this.SparnaturalComponents.findParentOrSiblingCriteria(this.parentCriteriaGroup.thisForm_, this.parentCriteriaGroup.id) ;
...
export class ObjectPropertyGroup extends GroupContenaire {
	...
	constructor(parentCriteriaGroup, specProvider, settings, temporaryLabel) {
		...
		this.objectPropertySelector = new this.SparnaturalComponents.ObjectPropertyTypeId(this, specProvider) ;
...
export class EndClassGroup extends GroupContenaire {
	...
	onStartClassGroupSelected() {
		...
		$(this.html).find('select.input-val').on('change', {arg1: this, arg2: 'onChange'}, this.SparnaturalComponents.eventProxiCriteria);
...
export class ObjectPropertyTypeId extends HTMLComponent {
	...
	init(reload = false) {
		...
		if(this.parentComponent.parentCriteriaGroup.jsonQueryBranch != null) {
			default_value = this.parentComponent.parentCriteriaGroup.jsonQueryBranch.line.p ;
			this.needTriggerClick = true ;
		}
...
export class VariableSelector extends HTMLComponent {
	...
	remove () {
		...
		this.GroupContenaire.parentComponent.thisForm_.queryOptions.displayVariableList = this.GroupContenaire.parentComponent.thisForm_.queryOptions.displayVariableList.filter(function(value){
